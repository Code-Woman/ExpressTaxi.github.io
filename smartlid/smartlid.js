...
(function ($) {
	var defaults = {
		...
		//** Включить форму c контактами? true|false
		//** contactForm: false,
		...
		callFormBtnIcon: 'Заказать звонок',
	...
	var methods = {
		...
		createButtons: function () {
			...
			let smartLid = $('<div>').attr({
				class: 'smartlid' + defaults.position + ' ' + 'smartlid_visible',
			});
			...
			// Для вызова модального окна своей ссылкой c номером телефона
			if (defaults.callForm) {
			...
			// Для вызова своей ссылкой модального окна c отправкой сообщения
			if (defaults.requestForm) {
